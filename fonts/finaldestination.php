...
<table class="theader">
<td>
<?php echo"<a href=\"./index.php?ruolo=".$_GET['ruolo']."\"> Lista Attrezzature </a>" ?>
</td>
<td>
<?php//if($ruolo=$_GET['ruolo']!=""&&$ruolo=$_GET['ruolo']=="administrator"||$ruolo=$_GET['ruolo']=="generalmanager"||$ruolo=$_GET['ruolo']=="amministratoreunico"||$ruolo=$_GET['ruolo']=="supervisore"){ echo"<a href=\"".$href.$_GET['ruolo']."\"> Inserisci Attrezzatura </a>";}?>
</td>
<td>
<?php if($ruolo=$_GET['ruolo']!=""&&$ruolo=$_GET['ruolo']=="administrator"||$ruolo=$_GET['ruolo']=="generalmanager"||$ruolo=$_GET['ruolo']=="amministratoreunico"||$ruolo=$_GET['ruolo']=="supervisore"){ echo"<a href=\"".$hreftwo.$_GET['ruolo']."\"> Vai a Macchine Destinate </a>";}?>
</td>
<!--<td>
...
if (isset($_POST['cerca'])){
	...
	//else{
		if($modello==""&&$formato==""&&$colore==""&&$condizione==""&&$tipologia==""){$sql = "SELECT * FROM `Sql1062326_3`.`destinate`;";}
		else{$sql = "SELECT * FROM `Sql1062326_3`.`destinate` WHERE  ".$modello." ".$tipologia." ".$formato." ".$colore." ".$condizione." ;";}
		...
		echo $tableheader;
			...
			echo"<td >".rawurldecode($userRow['NOTE'])." </td>";
			echo"<td ><a href=./convertfpdf.php?ID=".$userRow['ID']." target=”popup”><button type=”submit”>Apri Scheda</button></form></td>";
			//if($ruolo=!""&&$ruolo=="administrator"){echo"<td ><a href=./editreport.php?ID=".$userRow['ID']."&ruolo=".$_GET['ruolo']." ><button type=”submit”>Modifica Record</button></form></td>";}
			//if($ruolo=!""&&$ruolo=="administrator"){echo"<td ><a href=./deletereport.php?ID=".$userRow['ID']."&ruolo=".$_GET['ruolo']." ><button type=”submit”>Elimina Record</button></form></td>";}
			if($ruolo=!""&&$ruolo=="administrator"||$ruolo=="generalmanager"||$ruolo=="amministratoreunico"||$ruolo=="supervisore"){echo"<td ><a href=./finaldestination.php?ID=".$userRow['ID']."&ruolo=".$_GET['ruolo']."&azione=sposta ><button type=”submit”>Sposta in magazzino</button></form></td>";}
			echo"</tr>"; 
			}
		echo "<table>";}else{if(isset($_POST['multiple'])){$passaruolo=$ruolo;header("refresh:1;url=./querytwo.php?ruolo=".$_GET['ruolo']);}}
		...
		$id=$_GET['ID'];
		$sql3 = "SELECT * FROM `Sql1062326_3`.`destinate` WHERE `ID`=".$id." LIMIT 1;";
		...
		$sql4 = "DELETE FROM `Sql1062326_3`.`destinate` WHERE `ID`=".$id." LIMIT 1;";
		$result4=mysql_query($sql4);
		echo "<span style=\"color:#2db534;\">Record numero ".$_GET['ID']." Spostato in Magazzino!</'span>";
		}}
