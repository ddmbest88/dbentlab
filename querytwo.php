...
$ruolo=$_GET['ruolo'];
$sql = "SELECT modello FROM destinate GROUP BY modello HAVING count(*) >= 1;";
...
header("refresh:0.5;url=./finaldestination.php?datidacercare=".$data."&ruolo=".$ruolo."&username=".$username);	
}else{
	if(isset($_POST['back'])){
		header("refresh:1;url=./finaldestination.php?ruolo=".$ruolo."&username=".$username);
	}
}
